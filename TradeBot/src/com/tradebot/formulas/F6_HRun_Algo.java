...
public class F6_HRun_Algo 
{
	...
	public F6_HRun_Algo(presto_commons objconnect,  String feedid, double tradeprice, int asksize,int bidsize ,String tickdatetime) 
	{
		...
		objPresto =objconnect;
		 try
		 {
			 ...
			 for(int i =0; i < tradeplayers.size(); i++)
			 {
				 ...
				 {
					 if (dbObj.getRowCount("SELECT * FROM TBL_F6_HRUN_TRADES  WHERE FEEDSECID='"+feedid+"' and TRADESECID='"+tradeplayers.get(i)+"'") == 0)// and ISBUYSELLDONE ='false'
					 {
						 dbObj.executeNonQuery("insert into TBL_F6_HRUN_TRADES (FEEDSECID, TRADESECID,ENTRYTIME,BUYPRICE,SELLPRICE,EXITTIME,ISSHOTSELL,HIGH,LOW,ISBOUGHT, ISSELL,MPOINT,EXITCONDITION,TCOUNT,ISBUYSELLDONE,ENTRYID,EXITID,C,LC,S) values "
						 		+ "('"+feedid+"', '"+tradeplayers.get(i)+"', null, 0.0,0.0,null,'false',0.0,0.0,'false','false',0.0,null,0,'false',null,null,0,0,0)");
					 }
					 if((dbObj.getRowCount("SELECT * FROM TBL_F6_HRUN_TRADES  WHERE FEEDSECID='"+feedid+"' and TRADESECID='"+tradeplayers.get(i)+"' and ISBUYSELLDONE ='true'") > 0) && (dbObj.getRowCount("SELECT * FROM TBL_F6_HRUN_TRADES  WHERE FEEDSECID='"+feedid+"' and TRADESECID='"+tradeplayers.get(i)+"' and ISBUYSELLDONE ='false'") != 1 ))
					 {
						String[][] lasttransac =  dbObj.getMultiColumnRecords("SELECT high, low, mpoint, C, LC, S FROM TBL_F6_HRUN_TRADES WHERE id = (SELECT MAX(id) FROM TBL_F6_HRUN_TRADES ) and FEEDSECID='"+feedid+"' and TRADESECID='"+tradeplayers.get(i)+"';");
  	...
  	public void assginF1Variables(String feedid, String tradeid, String livedate)
	{
		...
		try
		{
			...
			F1tradedata = dbObj.getMultiColumnRecords("SELECT * FROM TBL_F6_HRUN_TRADES where FEEDSECID='"+feedid+"' and TRADESECID='"+tradeid+"' and ISBUYSELLDONE='false'");
			if (F1tradedata != null)
			{
	...
	public void CalculateHRun(String feedid, String tradeid, Double tickprice, Date ticktime)
	{
		...
		try
		{
			...
			{
				alreadyEntered = dbObj.getRowCount("SELECT * FROM TBL_F6_HRUN_TRADES WHERE FEEDSECID = '"+feedid+"' and TRADESECID = '"+tradeid+"' and ISBUYSELLDONE ='true'");
				if (isBought == true)
				{
					// Box 2
					if(ticktime.after(t4))
					{
    	    			...
    	    			if (istradeswitch ==true)
    	    			{
    	    				if (bidvolume >= 1)
        	    			{
    	    					...
    	    					dbObj.executeNonQuery("UPDATE TBL_F6_HRUN_TRADES  SET EXITCONDITION='N/A',EXITTIME='"+ticktime.toString()+"',SELLPRICE ="+sellPrice+", "
    	    			...
    	    			{
    	    				dbObj.executeNonQuery("UPDATE TBL_F6_HRUN_TRADES  SET EXITCONDITION='N/A',EXITTIME='"+ticktime.toString()+"',SELLPRICE ="+sellPrice+", Tcount="+TCount+", ISBUYSELLDONE = 'true', LOW ="+low+", HIGH="+high+", Mpoint="+Mpoint+",C="+C+", LC="+LC+", S="+S+" WHERE FEEDSECID='"+feedid+"' and TRADESECID ='"+tradeid+"' and ISBUYSELLDONE ='false'");
    	    			}
					...
					else if(tickprice >= Mpoint + (Mpoint*(z/100)))
					{
						...
						C=C+1;
						dbObj.executeNonQuery("UPDATE TBL_F6_HRUN_TRADES SET LOW ="+low+", HIGH="+high+", Mpoint="+Mpoint+",C="+C+", LC="+LC+", S="+S+" WHERE FEEDSECID='"+feedid+"' and TRADESECID ='"+tradeid+"' and ISBUYSELLDONE ='false'");
					...
					else if(tickprice > (Mpoint + (Mpoint*(y/100))))
					{
    	    			...
    	    			if (istradeswitch ==true)
    	    			{
    	    				if (askvolume >= 1)
        	    			{
    	    					...
    	    					dbObj.executeNonQuery("UPDATE TBL_F6_HRUN_TRADES  SET EXITCONDITION='N/A',EXITTIME='"+ticktime.toString()+"',BUYPRICE ="+buyPrice+", "
    	    							+ "Tcount="+TCount+", ISBUYSELLDONE = 'true' , LOW ="+low+", HIGH="+high+", Mpoint="+Mpoint+",C="+C+", LC="+LC+", S="+S+",  EXITID='"+orderid+"' WHERE FEEDSECID='"+feedid+"' and TRADESECID ='"+tradeid+"' and ISBUYSELLDONE ='false'");
    	    					Logger.info("Trade Switch is ON, Order Palced : "+orderid);
    	    				}
    	    			}
    	    			...
    	    			{
    	    				dbObj.executeNonQuery("UPDATE TBL_F6_HRUN_TRADES  SET EXITCONDITION='N/A',EXITTIME='"+ticktime.toString()+"',BUYPRICE ="+buyPrice+", Tcount="+TCount+", ISBUYSELLDONE = 'true', LOW ="+low+", HIGH="+high+", Mpoint="+Mpoint+",C="+C+", LC="+LC+", S="+S+" WHERE FEEDSECID='"+feedid+"' and TRADESECID ='"+tradeid+"' and ISBUYSELLDONE ='false'");
    	    			}
    	    			funcom.calculatefigure(sellPrice, buyPrice, feedid,tradeid, Fname,dbtable);
    	    			//calculatefigure(feedid,tradeid);
					}
				...
				else if ((isBought==false)&&(isSell==false)&&(alreadyEntered==0))
	    	    {
					if ((low == 0.0) && (high == 0.0))
		    	    {
		    	    	low = tickprice;
		    	    	high =tickprice;
		    	    	dbObj.executeNonQuery("UPDATE TBL_F6_HRUN_TRADES SET LOW = "+low+" , HIGH ="+ high +""
		    	    			+ " WHERE FEEDSECID='"+feedid+"' and TRADESECID ='"+tradeid+"' and ISBUYSELLDONE ='false'");
		    	    }
					if(tickprice > high){high = tickprice;dbObj.executeNonQuery("UPDATE TBL_F6_HRUN_TRADES  SET HIGH ="+ high +" WHERE FEEDSECID='"+feedid+"' and TRADESECID ='"+tradeid+"' and ISBUYSELLDONE ='false'");}
					if(tickprice < low){low = tickprice;dbObj.executeNonQuery("UPDATE TBL_F6_HRUN_TRADES  SET LOW ="+ low +" WHERE FEEDSECID='"+feedid+"' and TRADESECID ='"+tradeid+"' and ISBUYSELLDONE ='false'");} 
					...
					if (tickprice > buylegprice)
	        	    {
        	    		...
        	    		if (fst1.after(t2))
        	    		{
        	    			...
        	    			if (istradeswitch == true)
        	    			{
	        	    			...
	        	    			{
        	    					orderid = funcom.LoadDataandOrder(objPresto, feedid, tradeid, "BUY");
        	    					dbObj.executeNonQuery("UPDATE TBL_F6_HRUN_TRADES  SET ENTRYTIME = '"+ticktime.toString()+"',"
        	    		...
        	    		if (fst1.after(t2))
        	    		{
        	    			...
        	    			//sell command later
	        	    			...
	        	    			if (istradeswitch ==true)
	        	    			{
		        	    			...
		        	    			{
	        	    					orderid = funcom.LoadDataandOrder(objPresto, feedid, tradeid, "SELL");
	        	    					dbObj.executeNonQuery("UPDATE TBL_F6_HRUN_TRADES  SET ISSHOTSELL='"+isShotsell+"', ENTRYTIME='"+ticktime.toString()+"',SELLPRICE="+sellPrice+","
